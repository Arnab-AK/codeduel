...
class SubmissionCreate(BaseModel):
    ...
    code: str
    language: str = "python"
    # Omit for a standalone practice submission (phase 1's original use
    # case), or provide to attach this submission to a live duel -- which
    # is what makes the judge publish a live progress update to the
    # match's WebSocket room. Note there's no player_id field: the caller
    # is always the authenticated user making the request (see
    # routes_submissions.py), never a client-supplied value.
